...
TEST="3D_TaylorGreenVortex"
DESCRIPTION="MFC test run test $TEST"
#####################################
...
cd libs/clone/MFC
./mfc.sh run examples/$TEST/case.py --case-optimization -n $SLURM_NTASKS --binary mpirun | tee -a $OUTFILE
#################
